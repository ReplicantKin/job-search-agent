...
3. Use `source-check status` for each planned source URL before opening it. Skip a URL that is fresh within 24 hours unless the user changed a search preference or explicitly requests a fresh check. Use the approved browser/search surface to inspect company career pages and official ATS pages first, then recruiting feeds, BOSS 直聘, 猎聘, and 前程无忧. 优先公司官网和官方 ATS，完成检查后用 `source-check record` 保存每个已检查 URL、时间、结果数量和警告，再用 `ingest` 规范化 JSON/HTML。Source-check history is separate from job screening and never proves a role is still open.
